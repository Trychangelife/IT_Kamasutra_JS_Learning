...
yauhen; // {name: "Yaugen", age: 31, nickName: "WebDev"}


// Class types modificators
class User {

    public name: string;  // Свободный доступ к свойству или методу
    private nickName: string; // Элемент недоступен за пределами класса
    protected age: number; // Доступ к элементам могут получить только наследники
    readonly pass: number; // Элемент доступный только для чтения
